...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = getPost("philosophy", slug);
  if (!post) return {};
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE.siteUrl}/philosophy/${slug}`,
    },
    openGraph: {
...
export default async function PhilosophyPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = getPost("philosophy", slug);
  if (!post) notFound();

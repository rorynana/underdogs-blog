...
- `skills/verify-history.md` — 내러티브 히스토리 검증 스킬
...
2. **제목 후보 생성**
   - `skills/draft-post.md`의 제목 규칙에 따라 title/subtitle 후보 5개 생성
   - 사용자 선택 후 frontmatter에 반영

3. **★ 체크포인트 2: 초안 컨펌**
   - 초안 전문을 사용자에게 제시
   - 수정 요청 반영 (톤, 내용, 구조 등)
   - 사용자 "확정" 후 Phase 2.5로 진행
   - 수정이 필요하면 반영 후 재제시 (최대 2회)

### Phase 2.5: Verify History (내러티브 검증)

1. **히스토리 검증**
   - `skills/verify-history.md` 프로세스에 따라 실행
   - 초안의 사건/동기/수치/감정/인물 스토리를 표로 추출
   - 사용자에게 하나씩 ✅/❌ 확인 요청

2. **★ 체크포인트 2.5: 내러티브 컨펌**
   - ❌ 항목은 사용자가 알려준 실제 히스토리로 교체
   - 톤앤매너(사용자 고유 화법) 반영 확인
   - 모든 항목 ✅ 후 Phase 3로 진행
...
3. **플레이스홀더 교체**
   ...
   - OG 이미지: thumbnail 필드 있으면 커스텀 이미지, 없으면 `/api/og` Route 자동 fallback
